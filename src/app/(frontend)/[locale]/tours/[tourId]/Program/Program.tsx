...
//       <h2 className={styles.sectionTitle}>{tour.programTitle}</h2>
...
export default function Program({ tour, locale }: { tour: any; locale: string }) {
  ...
  const itinerary = tour?.itinerary || []
  const programTitle = t(tour?.programTitle) || (locale === 'en' ? 'Program' : 'Програма')

  return (
    <section className={styles.programSection}>
      <div className={styles.programLayoutInner}>
        {/* Ліва колонка (Sticky) - Умови */}
        <div className={styles.stickyWrapper}>
          <div className={styles.consultCard}>
            <div className={styles.consultHeader}>
              <h3>{locale === 'en' ? 'Booking Conditions' : 'Умови бронювання'}</h3>
        ...
        {/* Права колонка - Програма по днях */}
        <div className={styles.itineraryColumn}>
          {/* Фірмовий заголовок як у "Найближчі тури" */}
          <div className={styles.titleWrapper}>
            <h2 className={styles.mainTitle}>{programTitle}</h2>
          </div>
